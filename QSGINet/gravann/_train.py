...
from ._sample_observation_points import get_target_point_sampler,read_data
from ._io import load_sample, save_results, save_plots,save_csv
...
from gravann.networks.net import Network
# from .networks.sirenc import SingleBVPNet
...
def init_network(encoding, n_neurons=100, activation=nn.Sigmoid(), model_type="default", siren_omega=30., hidden_layers=9):
    ...
    elif model_type == " QSGINet":
        return Network(in_features=encoding.dim, decoder_hidden_dim=n_neurons, nl='sine',
                          encoder_type='none', decoder_n_hidden_layers=hidden_layers,init_type='siren', neuron_type='quadratic')
...
def _init_training_run(cfg, sample, lr, loss_fn, encoding, batch_size, target_sample_method, activation, omega, hidden_layers, n_neurons):
    ...
    group_else = list(map(lambda x: x[1], list(
        filter(lambda kv: ('r.' not in kv[0]) & ('g.' not in kv[0]) & ('b.' not in kv[0]), model.named_parameters()))))

    optimizer = torch.optim.Adam([
        {"params": group_r},
        {"params": group_g, "lr": 0.00001},
        {"params": group_b, "lr": 0.00001},
        {"params": group_else}, ],
        lr,
        weight_decay=0.0001)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, factor=0.8, patience=200, min_lr=1e-10, verbose=True)
...
def run_training(cfg, sample, loss_fn, encoding, batch_size, target_sample_method, activation, omega, hidden_layers, n_neurons):
    ...
    bt = int(cfg["training"]["iterations"])
    caiyang = []
    ...
    for it in t:
        ...
        if (it % 100 == 0):
            ...
            plot_model_rejection(model, encoding, views_2d=True, bw=True, N=cfg["plotting_points"], alpha=0.1,
                                 s=50, save_path=run_folder + "rejection_plot_iter" + format(it, '06d') + ".png",
                                 save_data=run_folder + "rejection_data_iter" + format(it, '06d') + ".pk",c=c, progressbar=False)
            plot_model_vs_mascon_contours(model, encoding, mascon_points, N=cfg["plotting_points"],
                                          save_path=run_folder + "contour_plot_iter" + format(it, '06d') + ".png",
                                          save_data=run_folder + "contour_data_iter" + format(it, '06d'),c=c)
            plt.close('all')
        # Each ten epochs we resample the target points
        if (it % 10 == 0):
            target_points = targets_point_sampler()
            caiyang.append(target_points)
    ...
    # validation_results, diy, diz, dic, gay, gaz, gac, buy, buz, buc = validation(
    #     model, encoding, mascon_points, mascon_masses_u,
    #     cfg["model"]["use_acceleration"], "3dmeshes/" + sample,
    #     mascon_masses_nu=mascon_masses_nu,
    #     N_integration=500000, N=cfg["training"]["validation_points"])
    ...
    # 保存为cvs数据
    # 保存1.25r结果
    # save_csv(dic, diy, run_folder + "model1.25r" + ".csv")

    # 保存1.5r结果
    # save_csv(gac, gay, run_folder + "model1.5r" + ".csv")

    # 保存1.75r结果
    # save_csv(buc, buy, run_folder + "model1.75r" + ".csv")
    ...
    cfg_dict = {"Sample": sample, "Type": "ACC" if cfg["model"]["use_acceleration"] else "U", "Model": cfg["model"]["type"],  "Loss": loss_fn.__name__, "Encoding": encoding.name,
                ...
                "Integration Points": cfg["integration"]["points"], "Vision Loss": cfg["training"]["visual_loss"], "c": c.cpu().detach().numpy()}
...
#         model = init_network(
#             encoding, model_type=params["Model"], activation=activation, n_neurons=params["n_neurons"], hidden_layers=params["hidden_layers"], siren_omega=30)
#     else:
#         model = init_network(
#             encoding, model_type=params["Model"], activation=activation, siren_omega=30)
#     model.load_state_dict(torch.load(folderpath + "best_model.mdl",map_location='cuda:0'))
#
...
def load_model_run(folderpath, differential_training=False):
    ...
    if "n_neurons" in params and "hidden_layers" in params:  # newer cfgs have these entries
        model = init_network(
            encoding, model_type=params["Model"], activation=activation, n_neurons= int(100), hidden_layers=int(params["hidden_layers"][0]), siren_omega=30)
    else:
        model = init_network(
            encoding, model_type=params["Model"], activation=activation, siren_omega=30)
    model.load_state_dict(torch.load(folderpath + "best_model.mdl",map_location='cuda:0'))
