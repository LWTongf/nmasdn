...
def compute_c_for_model(model, encoding, mascon_points, mascon_masses, mascon_masses_nu=None, use_acc=True):
    ...
    mm=(torch.sum(predicted*labels)/torch.sum(predicted*predicted)).item()
...
#     for idx, altitude in enumerate(sampling_altitudes):
#         torch.cuda.empty_cache()
#         pred, labels, loss_values, tg = [], [], [], []
#         target_sampler = get_target_point_sampler(
#             N=batch_size, method="altitude",
#             bounds=[altitude], limit_shape_to_asteroid=asteroid_pk_path)
#         for batch in range(N // batch_size):
#             target_points = target_sampler().detach()
#             tg.append(target_points)
#             labels.append(label_function(
#                 target_points, mascon_points, mascon_masses).detach())
#
#             prediction = integrator(target_points, model, encoding, N=N_int,
...
#
#     torch.cuda.empty_cache()
#     return results, diy, diz, dic, gay, gaz, gac, buy, buz, buc
...
#     pred, labels, loss_values = [], [], []
#     # 读入第一个csv数据，1.25r
#     f = "/root/tt/smapefg/sampe1.25(km)r.csv"
#     target_points = read_data(f)
#     target_points = torch.tensor(target_points).detach()
#
#     dic.append(target_points)
#     print("开始验证1.25r高度加速度#########################")
#     if progressbar:
#         pbar = tqdm(desc="Computing validation 1.25r...",
...
#
#     results = results._append(
#         dict(zip(cols, ["1.25r Altitude"] + loss_values)), ignore_index=True)
#
#     print("1.25r加速度验证结束######################################################")
#
#     # 1.5r距离采样计算
#     torch.cuda.empty_cache()
#     pred, labels, loss_values = [], [], []
#     f1 = "/root/tt/smapefg/sampe1.5(km)r.csv"
#     target_points = read_data(f1)
#     target_points = torch.tensor(target_points).detach()
#     gac.append(target_points)
#
#     print("开始验证1.5r高度加速度#########################")
#
#     if progressbar:
#         pbar = tqdm(desc="Computing validation 1.5r...",
#                     total=len(target_points))
#     for idx in range((len(target_points) // batch_size) + 1):
#         indices = list(range(idx * batch_size,
#                              np.minimum((idx + 1) * batch_size, len(target_points))))
...
#
#     results = results._append(
#         dict(zip(cols, ["1.5r Altitude"] + loss_values)), ignore_index=True)
#     print("1.5r验证结束#########################")
#
#     ################################################
#     # 1.75r距离采样计算
#     torch.cuda.empty_cache()
#     pred, labels, loss_values = [], [], []
#     f2 = "/root/tt/smapefg/sampe1.75(km)r.csv"
#     target_points = read_data(f2)
#     target_points = torch.tensor(target_points).detach()
#     buc.append(target_points)
#
#     print("开始验证1.75r高度加速度#########################")
#
#     if progressbar:
#         pbar = tqdm(desc="Computing validation 1.75r...",
#                     total=len(target_points))
#     for idx in range((len(target_points) // batch_size) + 1):
#         indices = list(range(idx * batch_size,
#                              np.minimum((idx + 1) * batch_size, len(target_points))))
...
#
#     results = results._append(
#         dict(zip(cols, ["1.75r Altitude"] + loss_values)), ignore_index=True)
#     print("1.75r验证结束#########################")
#
#     torch.cuda.empty_cache()
#     return results, diy, diz, dic, gay, gaz, gac, buy, buz, buc
def validation(model, encoding, mascon_points, mascon_masses,
    ...
    target_points = target_sampler().detach()
    print("弟高度采样点：",len(target_points))
    ...
    target_points = target_sampler().detach()
    print("gao高度采样点：", len(target_points))
    for idx in range((len(target_points) // batch_size)+1):
        indices = list(range(idx*batch_size,
                             np.minimum((idx+1)*batch_size, len(target_points))))
        points = target_points[indices]
